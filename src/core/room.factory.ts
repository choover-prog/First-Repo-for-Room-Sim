...
import { ROOM_CHANGED, roomStore } from '../state/room.slice';
import { attachRoom } from './room.session';

export async function buildRoomFromPreset(
  presetId: string,
  ctx: { scene: THREE.Scene; camera: THREE.PerspectiveCamera; controls: any; bus: any }
) {
  ...
  ctx.bus.emit(ROOM_CHANGED, { presetId, width, depth, height });
